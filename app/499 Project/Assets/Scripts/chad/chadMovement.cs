...
public class chadMovement : MonoBehaviour
{
    ...
    {   
        if(collision.gameObject.CompareTag("Enemy") && (chargePlayer || followPlayer))
        {     
            ResetWaitTime(1f, 2f);
            chargeDuration = 0.75f;
        }
        else if((collision.gameObject.CompareTag("Wall") || collision.gameObject.CompareTag("BrokenWall") || collision.gameObject.CompareTag("Breakable") || collision.gameObject.CompareTag("Enemy")) && !chargePlayer && !followPlayer)
        {
